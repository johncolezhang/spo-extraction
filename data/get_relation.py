...
for seq in D:
    ...
    for subj in subjects:
        for obj in objects:
            ...
            if select_subj_obj:
                content += str(relation_dict[select_subj_obj[0][1]]) + ' ' + subj + '$' + obj + '$' + \
                           seq["text"].replace(subj, '#' * len(subj)).replace(obj, "#" * len(obj)) + '\n'
            else:
                content += str(0) + ' ' + subj + '$' + obj + '$' + \
                           seq["text"].replace(subj, '#' * len(subj)).replace(obj, "#" * len(obj)) + '\n'

with open("train_relation.txt", "w", encoding="utf-8") as f:
    f.write(content)
...
with open("test_relation.txt", "w", encoding="utf-8") as f:
    f.write(content)
